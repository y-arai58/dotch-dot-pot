...
export async function generateCharacter(request:GenerationRequest,references:{side:string;data:string}[],directory:string,signal:AbortSignal,progress:(n:number,activity?:GenerationActivity)=>void):Promise<CreationArtifact>{
 ...
 const rpc=new CodexRpc();try{
  ...
  let candidate:unknown=JSON.parse(await rpc.turn(threadId,[explicitSkill,textInput(prompt),...references.map(r=>({type:'image',url:r.data}))],isAnimal?animalModelOutputSchema:modelOutputSchema,signal,turnOptions));
  ...
  for(attempt=0;attempt<3;attempt++){
   ...
   const review=JSON.parse(await rpc.turn(threadId,input,isAnimal?animalReviewOutputSchema:reviewOutputSchema,signal,turnOptions));
