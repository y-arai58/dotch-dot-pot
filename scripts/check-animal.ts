...
import {inspectAnimal} from './animal-quality';
...
const result=await inspectAnimal(JSON.parse(await readFile(modelPath,'utf8')),style,out);console.log(JSON.stringify(result.validation,null,2));process.exitCode=result.validation.issues.length?1:0;
