...
export type Revision = {id:string;rendererVersion?:string;createdAt:string;style:Style;frames:Frame[];baseFrames:Frame[];approved:boolean;reviewed:boolean;issues:string;mode:'front'|'eight';source:'sample'|'tripo'|'import'|'skill';rigKind?:'humanoid'|'quadruped'|'prop';modelId:string;features:string[];name:string;prompt:string;facing:number;size:number;modelKey?:string;referenceKeys?:string[];sharedEdits?:SharedEdits;parentRevisionId?:string;reason?:RevisionReason};
export type RevisionReason='sample'|'skill'|'import'|'duplicate'|'restyle'|'regenerate'|'redraw-direction'|'extend-eight'|'shared-edit'|'part-edit'|'articulated';
