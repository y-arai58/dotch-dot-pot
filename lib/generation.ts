...
import {buildMesh,renderSet,validateFrames,type Style} from './pixel';
...
export const CREATION_SKILLS=[{id:'humanoid',name:'人型キャラクター',skillName:'dotforge-humanoid',version:'1.0.0',artifactKind:'rigged-humanoid-v1',validationFrames:344},{id:'animal',name:'四足動物',skillName:'dotforge-animal',version:'1.0.0',artifactKind:'rigged-quadruped-v1',validationFrames:376}] as const;
export const generationRequestSchema=z.object({id:idSchema,projectId:idSchema,skillId:z.enum(['humanoid','animal']),name:z.string().min(1).max(100),prompt:z.string().min(1).max(2000),features:z.array(z.string().min(1).max(150)).max(10),mode:z.enum(['front','eight']),style:styleSchema,referenceKeys:z.array(idSchema).max(3),referenceSides:z.array(z.enum(['front','left','back','right'])).max(3)}).strict();
...
export function staticModelIssues(model:RiggedModel|AnimalModel,style:Style){return validateFrames(renderSet(buildMesh(model),style,'eight'),style,'eight');}
...
export type CreationArtifact=HumanoidArtifact|AnimalArtifact;
export function parseCreationModel(artifact:Pick<CreationArtifact,'kind'|'model'>){if(artifact.kind==='rigged-humanoid-v1')return parseHumanoid(artifact.model);if(artifact.kind==='rigged-quadruped-v1')return parseAnimal(artifact.model);throw Error('未対応のモデル形式です');}
