...
export const CREATION_SKILLS=[{id:'humanoid',name:'人型キャラクター',skillName:'dotforge-humanoid',version:'1.0.0',artifactKind:'rigged-humanoid-v1'}] as const;
export const generationRequestSchema=z.object({id:idSchema,projectId:idSchema,skillId:z.literal('humanoid'),name:z.string().min(1).max(100),prompt:z.string().min(1).max(2000),features:z.array(z.string().min(1).max(150)).max(10),mode:z.enum(['front','eight']),style:styleSchema,referenceKeys:z.array(idSchema).max(3),referenceSides:z.array(z.enum(['front','left','back','right'])).max(3)}).strict();
...
export function staticModelIssues(model:RiggedModel,style:Style){return validateFrames(renderSet(buildMesh(model),style,'eight'),style,'eight');}
