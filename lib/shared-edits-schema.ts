...
export const sharedEditsSchema=z.object({
 ...
 parts:z.record(z.string().min(1).max(200),z.object({offset:vector(-3,3),scale:vector(.1,3),rotation:vector(-180,180).optional(),pivot:vector(-5,5).optional()}).strict()).refine(parts=>Object.keys(parts).length<=200),
