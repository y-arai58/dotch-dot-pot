...
const propTerms:Record<string,string>={tabletop:'天板',underframe:'下枠',body:'本体',chest:'宝箱',leg:'脚',legs:'脚',apron:'幕板',brace:'補強桟',seat:'座面',frame:'枠',backrest:'背もたれ',post:'支柱',slat:'桟',rail:'横木',cushion:'クッション',lid:'蓋',latch:'留め具',hinge:'蝶番',handle:'取っ手',drawer:'引き出し',housing:'外枠',knob:'つまみ',mount:'台座',brass:'真鍮',book:'本',cover:'表紙',page:'ページ',block:'束',spine:'背表紙',inset:'飾り',top:'上部',bottom:'底',base:'土台',rim:'縁',band:'帯',barrel:'樽',lamp:'照明',shade:'傘',stem:'支柱',wood:'木部',wooden:'木部',metal:'金具',side:'側面'};
export function partLabel(part:{id:string;name:string},revision:Pick<Revision,'name'|'prompt'|'features'|'rigKind'>,index=0){
 ...
 if(revision.rigKind==='prop'){
  const labels=tokens.filter(t=>!prefixes.has(t)).map(t=>propTerms[t]||terms[t]||(/^\d+$/.test(t)?t:undefined)).filter(Boolean);
  const location=[side,front?'前側':hind?'後ろ側':''].filter(Boolean).join('・');
  return labels.length?(location?location+'の':'')+[...new Set(labels)].join('・'):part.name;
 }
