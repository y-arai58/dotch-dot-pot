...
const clip=z.object({id:motionId,name:z.string().min(1).max(40),frames:z.number().int().min(4).max(24),fps:z.number().int().min(4).max(24),loop:z.boolean(),stride:z.number().min(0).max(.6),lift:z.number().min(0).max(.3),depth:z.number().min(0).max(.4),height:z.number().min(0).max(.6),tailSwing:z.number().min(0).max(35),hold:z.number().min(.1).max(.7),keys:z.record(boneId,z.array(z.object({frame:z.number().int().min(0).max(23),rotation:angles})).max(24))});
