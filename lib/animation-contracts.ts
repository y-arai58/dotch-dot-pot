...
export const animationSchema=z.object({id:idSchema,assetId:idSchema,sourceRevisionId:idSchema,name:z.string().min(1).max(100),config:z.object({version:z.literal(MOTION_VERSION),rig:z.object({bones:z.array(z.object({id:boneId,parent:boneId.optional(),pivot:vector})).length(16),bindings:z.record(z.string().max(200),boneId),reviewed:z.boolean(),origin:z.enum(['sample','embedded','manual','skill']),skinning:z.enum(['envelope','embedded','rigid']).optional()}),clips:z.array(clip).min(1).max(4),scale:z.number().min(.1).max(2),facing:z.number().min(0).max(315).multipleOf(45),style:styleSchema,mode:z.enum(['front','eight'])}),baked:z.array(z.object({id:motionId,frames:z.array(z.array(packed).min(1).max(8)).min(4).max(24),issues:z.array(z.string().max(300)).max(1000),events:z.array(z.object({frame:z.number().int().min(0).max(23),name:z.string().max(50)})).max(100)})).max(4),version:z.number().int().min(0),updatedAt:z.string(),reviewed:z.boolean()});
