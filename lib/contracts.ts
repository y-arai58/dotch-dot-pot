...
export const revisionSchema=z.object({id:idSchema,rendererVersion:z.string().max(100).optional(),createdAt:z.string().datetime(),style:styleSchema,frames:z.array(frame).min(1).max(8),baseFrames:z.array(frame).min(1).max(8),approved:z.boolean(),reviewed:z.boolean(),issues:z.string().max(2000),mode:z.enum(['front','eight']),source:z.enum(['sample','tripo','import','skill']),rigKind:z.enum(['humanoid','quadruped']).optional(),modelId:z.string().max(100),features:z.array(z.string().max(250)).max(20),name:z.string().min(1).max(100),prompt:z.string().max(2000),facing:z.number().min(0).max(315).multipleOf(45),size:z.number().min(.25).max(2),modelKey:idSchema.optional(),referenceKeys:z.array(idSchema).max(3).optional(),sharedEdits:sharedEditsSchema.optional(),parentRevisionId:idSchema.optional()});
