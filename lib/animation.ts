...
export type Rig={bones:Bone[];bindings:Record<string,BoneId>;reviewed:boolean;origin:'sample'|'embedded'|'manual';skinning?:'envelope'|'embedded'|'rigid';};
...
export function rigFromModel(model:RiggedModel):Rig{return {bones:clone(model.rig.bones),bindings:Object.fromEntries(model.parts.map(p=>[p.id,p.bone])),reviewed:true,origin:'sample'};}
