...
export function qFromTo(a:V3,b:V3):Quat{a=unit(a);b=unit(b);const d=clamp(dot(a,b),-1,1);if(d<-.999999){let axis=cross(a,[1,0,0]);if(length(axis)<.001)axis=cross(a,[0,1,0]);axis=unit(axis);return [...axis,0];}const c=cross(a,b),q:Quat=[...c,1+d],n=Math.hypot(...q)||1;return q.map(v=>v/n) as Quat;}
export function qEuler(v:V3):Quat{const r=v.map(n=>n*Math.PI/360);return qMultiply(qMultiply([0,0,Math.sin(r[2]),Math.cos(r[2])],[0,Math.sin(r[1]),0,Math.cos(r[1])]),[Math.sin(r[0]),0,0,Math.cos(r[0])]);}
...
export function solveLeg(hip:V3,target:V3,upper:number,lower:number,poleDirection:V3=[0,-1,0]){const delta=sub(target,hip),distance=length(delta),min=Math.abs(upper-lower)+1e-6,max=upper+lower-1e-6,d=clamp(distance,min,max),axis:V3=distance<1e-8?[0,0,-1]:unit(delta);let pole=sub(poleDirection,mul(axis,dot(poleDirection,axis)));if(length(pole)<1e-6)pole=[0,0,1];pole=unit(pole);const along=(upper*upper-lower*lower+d*d)/(2*d),out=Math.sqrt(Math.max(0,upper*upper-along*along));return {knee:add(hip,add(mul(axis,along),mul(pole,out))),ankle:add(hip,mul(axis,d)),unreachable:distance>max+.0001||distance<min-.0001};}
