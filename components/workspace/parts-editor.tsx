...
type Props={open:boolean;locked:boolean;onDirtyChange:(dirty:boolean,discard:()=>void)=>void;revision:Revision;direction:Direction;loadBaseMesh:(r:Revision)=>Promise<Mesh>;onApply:(edits:SharedEdits,preview:{frames:Frame[];baseFrames:Frame[]})=>Promise<void>};
...
/** Part design editor shown as the workspace's パーツ tab. Keeps its edits while the tab is hidden. */
export function PartsEditor({open,locked,onDirtyChange,revision,direction,loadBaseMesh,onApply}:Props){
 ...
 useEffect(()=>{if(!open||loadedFor.current===revision.id)return;loadedFor.current=revision.id;let dead=false;setBusy(true);setBase(null);setError('');setUndo([]);setRedo([]);setMode('select');setIsolated(false);setView(direction);setStrokePixels(null);drag.current=null;
  loadBaseMesh(revision).then(mesh=>{if(dead)return;setBase(mesh);setPart(mesh.parts?.[0]?.id||'');setEdits(clone(revision.sharedEdits||emptySharedEdits(mesh)));}).catch(e=>{if(!dead){loadedFor.current='';setError(e instanceof Error?e.message:'パーツを読み込めません');}}).finally(()=>{if(!dead)setBusy(false);});return()=>{dead=true;};
 },[open,revision.id]);
 ...
 function discard(){if(!base)return;const next=clone(revision.sharedEdits||emptySharedEdits(base));draft.current=next;setEdits(next);setUndo([]);setRedo([]);setStrokePixels(null);}
 const discardRef=useRef(discard);useEffect(()=>{discardRef.current=discard;});
 const stableDiscard=useCallback(()=>discardRef.current(),[]);
 useEffect(()=>{onDirtyChange(changed,stableDiscard);},[changed,onDirtyChange,stableDiscard]);
 ...
 async function save(){if(!result||!changed)return;setBusy(true);setError('');try{await onApply(clone(edits),result);}catch(e){setError(e instanceof Error?e.message:'保存できませんでした。編集内容は残っています');}finally{setBusy(false);}}
 return <section className="part-editor-page"><header className="tab-heading"><div><h2><Box size={18}/>パーツを編集</h2><p>パーツの色・模様・形と向きを調整し、元の版を残して新しい候補に保存します。</p></div></header>
 ...
 {revision.rigKind!=='prop'&&<p className="help">ここでの支点は見た目を回す基準です。歩行などの関節は「モーション」で調整します。</p>}
 ...
 <footer className="shared-edit-footer"><button className="button" disabled={busy||!changed} onClick={discard}>変更を戻す</button><button className="button primary" disabled={busy||locked||!result||!changed||issues.length>0} onClick={()=>void save()}>{busy?<Loader2 className="spin" size={16}/>:<Check size={16}/>}パーツ修正を新候補に保存</button></footer>
 </section>;
}
