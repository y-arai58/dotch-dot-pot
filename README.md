...
消しゴム、透明部分・自動輪郭への描画、影、今回の修正元以外の方向に残っている手描きは、全方向へ移せない画素数として表示します。それらは新候補でも元の方向に保持し、破棄しません。元の版は最後に保存した状態のまま残し、未保存の描き込み・共通修正・全方向の画素を新しい版としてR2へ保存します。新候補の追加と同時に元の版を書き換える保存は拒否します。キャンバスの「保存」で先に確定した描き込みは、その版の保存済み状態として扱います。採用済みの版は複製してから編集します。
