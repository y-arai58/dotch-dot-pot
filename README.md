...
静止画を鉛筆や塗りつぶしで修正し、キャンバス下の「手修正を全方向へ反映」を開きます。修正した方向の色・模様を、画素から見える最前面の三角形上の範囲として記録し、全方向の候補を表示します。修正元では選択したパレット色を再現し、他方向ではプロジェクトの光源に応じた陰影を付けます。背後に隠れた別の面や反対側へは複製しません。
