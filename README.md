...
素材の「アニメーション」から編集室を開きます。生成ボタンの直前に、部位の確認欄と生成できない理由を表示します。「部位・左右・支点を確認した」をチェックすると生成できます。支点や追従先を変えると再確認が必要です。採用済みの場合は「新しい候補」を作成してから編集します。体を16関節の階層とパーツの追従先で管理し、待機・歩行・しゃがみ・ジャンプの同じ姿勢を1方向または8方向へ描画します。
...
動物素材のアニメーション編集室では、四つの足の接地、関節と部品の追従先、回転キー、尻尾の向きと動きを編集できます。歩幅は接地中の足の移動距離です。書き出す推奨移動速度は `stride / (0.75 × duration)`、前方は−Yです。書き出し形式は人型と共通で、motionVersionは `quadruped-ik-v1`、イベント名は各pawのcontact/liftです。人型素材は従来の16関節と344枚の検証を維持します。
...
キャンバス下の「パーツを編集」で、一覧または画像をクリックしてパーツを選びます。見えないパーツは別方向から選ぶか「選択パーツだけ表示」を使います。パレットから地色を変え、「模様を描く」で選択した表面だけに描き込めます。別のパーツや背景は塗りません。模様の取り消し・やり直し、選択パーツの模様の消去に対応します。
...
8方向のプレビューを確認し「パーツ修正を新候補に保存」を押します。以前の版は最後に保存した状態のまま残り、色・模様・大きさ・位置・角度・支点は新候補に保存されます。採用済みの版は先に「候補を複製」を押します。パーツ編集で描いた模様は表面に保存されるため、回転や動作に追従します。通常キャンバスで描いた未共有の画素は元の方向・位置に保持されます。一緒に回す場合は先に「手修正を全方向へ反映」で共有します。

この支点は静止画のデザインを回す基準で、アニメーションの関節の支点とは別です。生き物は元の関節への追従を保持しますが、大きな変更後は動作編集室で接地・接続を確認して動作を再生成します。今回の形状編集は倍率・移動・回転で、頂点の自由変形、パーツの追加、文章による部分再生成は含みません。
